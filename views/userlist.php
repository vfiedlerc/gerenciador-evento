...
<div class="col-md-12">
  <div class="box">
	  ...
	  foreach($records as $rec) {
        ...
        <tr>
		  ...
		  <td><?php if($status == "active") {?>
            <a href="javascript:status('<?php echo $user_id ?>', 'inactive');">Inativo</a>&nbsp;/
			&nbsp;<a href="javascript:status('<?php echo $user_id ?>', 'lock');">Conta trancada</a>&nbsp;/
			&nbsp;<a href="javascript:status('<?php echo $user_id ?>', 'delete');">Deletar</a>
            <?php } else { ?>
			<a href="javascript:status('<?php echo $user_id ?>', 'active');">Ativar</a>
			<?php }//else ?>
          </td>
		  <?php }?>
        </tr>
	...
	if($type == 'admin') {
	?>
	<button type="button" class="btn btn-info" onclick="javascript:createUserForm();"><i class="fa fa-user-plus" aria-hidden="true"></i>&nbsp;Criar novo usuario</button>
	<?php 
	}
	?>
      <!--
